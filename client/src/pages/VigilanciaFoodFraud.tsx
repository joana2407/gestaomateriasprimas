...
export default function VigilanciaFoodFraud() {
  ...
  function guardarRelatorio() {
    ...
    const fonteSelecionada = fonteManual.match(/https?:\/\/\S+/)?.[0] ?? fonteManual;
    const fontes = Array.from(new Set([fonteSelecionada, ...results.flatMap(r => r.fontes)].filter(Boolean)));
    if (!fontes.length) return toast.error("Selecione a fonte utilizada antes de guardar o relatório.");
    ...
    guardar.mutate({ ficheiro: fileName || `food-fraud-${periodo}.txt`, periodoInicio: inicio.toISOString(), periodoFim: fim.toISOString(), totalAvaliados: results.length, totalRelevantes: relevantes.length, resumo: `Análise Food Fraud de ${periodo}: ${results.length} ocorrências avaliadas, ${relevantes.length} relevantes.`, ocorrencias: results, fontes });
  }
