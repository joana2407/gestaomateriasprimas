...
export default function Dashboard() {
  ...
  const mpsObsoletas = (materiasPrimas ?? []).filter((mp: any) => mp.categoria === "obsoleta");
  const mpsParaTestes = (materiasPrimas ?? []).filter((mp: any) => mp.categoria === "para_testes");
    ...
    >
      <div className="space-y-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          {(mpsObsoletas.length > 0 || mpsParaTestes.length > 0) && (
            <div className="lg:col-span-2 card-elegant p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">MP inativas ou reservadas para testes</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="text-center p-3 rounded-lg bg-red-50 border border-red-100">
                  <div className="text-2xl font-bold text-red-600">{mpsObsoletas.length}</div>
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {[...mpsObsoletas.map((mp: any) => ({ ...mp, estadoLabel: "Inativa", estadoClass: "bg-red-100 text-red-700 border-red-200" })), ...mpsParaTestes.map((mp: any) => ({ ...mp, estadoLabel: "Testes", estadoClass: "bg-blue-100 text-blue-700 border-blue-200" }))].slice(0, 8).map((mp: any) => (
                  <div key={`${mp.id}-${mp.estadoLabel}`} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-muted/30 border border-border/40">
                    <Package className="w-3.5 h-3.5 shrink-0 text-muted-foreground" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate">{mp.nome}</p>
                      {mp.codigo && <p className="text-[10px] text-muted-foreground">{mp.codigo}</p>}
                    </div>
