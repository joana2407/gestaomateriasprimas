...
export default function Dashboard() {
  ...
  const estadosOperacionaisMp = (materiasPrimas ?? []).flatMap((mp: any) =>
    ((mp.fabricasEstado ?? []) as Array<{ fabricaId: number; estado: "ativa" | "para_testes" | "inativa" }>).map(rel => ({ ...mp, ...rel }))
  );
  const mpsInativas = estadosOperacionaisMp.filter(mp => mp.estado === "inativa");
  const mpsParaTestes = estadosOperacionaisMp.filter(mp => mp.estado === "para_testes");
    ...
    >
      <div className="space-y-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {/* Alertas de categorização de MP */}
          {(mpsInativas.length > 0 || mpsParaTestes.length > 0) && (
            <div className="lg:col-span-2 card-elegant p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-sm font-semibold text-foreground">Estado das Matérias-Primas</h2>
                    <p className="text-xs text-muted-foreground">Estados operacionais por unidade fabril</p>
                  </div>
                </div>
                <Link href="/materias-primas">
                  <button className="flex items-center gap-1 text-xs text-primary hover:underline">
                    Ver matérias-primas <ArrowRight className="w-3 h-3" />
                  </button>
                </Link>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="text-center p-3 rounded-lg bg-red-50 border border-red-100">
                  <div className="text-2xl font-bold text-red-600">{mpsInativas.length}</div>
                  <div className="text-xs text-red-600 mt-0.5">Inativas</div>
                </div>
                <div className="text-center p-3 rounded-lg bg-blue-50 border border-blue-100">
                  <div className="text-2xl font-bold text-blue-600">{mpsParaTestes.length}</div>
                  <div className="text-xs text-blue-600 mt-0.5">Em testes</div>
                </div>
              </div>
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {[...mpsInativas.map((mp: any) => ({ ...mp, estadoLabel: "Inativa", estadoClass: "bg-red-100 text-red-700 border-red-200" })), ...mpsParaTestes.map((mp: any) => ({ ...mp, estadoLabel: "Testes", estadoClass: "bg-blue-100 text-blue-700 border-blue-200" }))].slice(0, 8).map((mp: any) => {
                  const fabrica = fabricas?.find(item => item.id === mp.fabricaId);
                  return <div key={`${mp.id}-${mp.fabricaId}-${mp.estadoLabel}`} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-muted/30 border border-border/40">
                    <Package className="w-3.5 h-3.5 shrink-0 text-muted-foreground" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate">{mp.nome}</p>
                      <p className="text-[10px] text-muted-foreground">{fabrica?.nome ?? `Fábrica #${mp.fabricaId}`}{mp.codigo ? ` · ${mp.codigo}` : ""}</p>
                    </div>
                    <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full border shrink-0", mp.estadoClass)}>{mp.estadoLabel}</span>
                  </div>;
                })}
              </div>
            </div>
