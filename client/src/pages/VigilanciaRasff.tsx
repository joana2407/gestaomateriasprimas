...
type ManualResultado = { linha: string; relevancia: "Direta" | "Indireta" | "Informativa"; correspondencias: string[]; url?: string; prioridade: PrioridadeAlerta; dataAlerta?: string; numeroAlerta?: string };
...
function sugestaoAprendida(item: ManualResultado, regras: RegrasPrioridade): PrioridadeAlerta | undefined {
  const chaves = [item.numeroAlerta ? `alerta:${item.numeroAlerta}` : "", ...termosParaAprendizagem(item).map(termo => `termo:${termo}`)].filter(Boolean);
...
function analisarFicheiro(texto: string, materiasPrimas: Array<Record<string, unknown>>): ManualResultado[] {
  return extrairLinhasFicheiro(texto).map(linha => {
    const normalizada = linha.toLocaleLowerCase("pt-PT");
    ...
    return { linha, relevancia, correspondencias, url: extrairUrlOrigem(linha), prioridade: extrairPrioridade(linha), dataAlerta: extrairDataAlertaRasff(linha), numeroAlerta: extrairNumeroAlerta(linha) };
  });
}
...
export default function VigilanciaRasff() {
  ...
  const resultadosComPrioridade = useMemo(() => manualFile?.results.map(item => ({ ...item, prioridade: sugestaoAprendida(item, regrasPrioridade) ?? item.prioridade })) ?? [], [manualFile, regrasPrioridade]);
  ...
  const editarPrioridade = (item: ManualResultado, prioridade: PrioridadeAlerta) => {
    ...
    setManualFile(anterior => anterior ? { ...anterior, results: anterior.results.map(registo => registo.linha === item.linha && registo.numeroAlerta === item.numeroAlerta ? { ...registo, prioridade } : registo) } : anterior);
  ...
  const guardarAnaliseManual = () => {
    ...
    const relevantes = manualFile.results.filter(item => item.relevancia !== "Informativa");
