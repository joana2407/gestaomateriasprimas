...
type ManualResultado = { id?: string; linha: string; relevancia: "Direta" | "Indireta" | "Informativa"; correspondencias: string[]; url?: string; prioridade: PrioridadeAlerta; dataAlerta?: string; numeroAlerta?: string };
type RegrasPrioridade = Record<string, PrioridadeAlerta>;

function relevanciaOperacional(item: ManualResultado): ManualResultado["relevancia"] {
  return item.prioridade === "Informativa" ? "Informativa" : item.relevancia;
}

function termosParaAprendizagem(item: ManualResultado): string[] {
  const texto = item.linha.toLocaleLowerCase("pt-PT").replace(/[^a-z0-9áàâãéêíóôõúçü\s]/gi, " ");
  return Array.from(new Set(texto.split(/\s+/).filter(termo => termo.length >= 5))).slice(0, 12);
}

function chaveAprendizagem(item: ManualResultado): string {
  return item.numeroAlerta ? `alerta:${item.numeroAlerta}` : `linha:${item.id ?? item.linha}`;
}

function sugestaoAprendida(item: ManualResultado, regras: RegrasPrioridade): PrioridadeAlerta | undefined {
  const chaves = [chaveAprendizagem(item)];
  const votos = new Map<PrioridadeAlerta, number>();
  chaves.forEach(chave => { const prioridade = regras[chave]; if (prioridade) votos.set(prioridade, (votos.get(prioridade) ?? 0) + 1); });
  const ordenadas = Array.from(votos.entries()).sort((a, b) => b[1] - a[1]);
...
function extrairNumeroAlerta(linha: string): string | undefined {
  const identificado = linha.match(/\b(?:rasff|alerta|notification)\s*(?:n(?:º|o|úmero)?[.\s:#-]*)?((?:20\d{2})[./-]\d{2,6})\b/i)?.[1];
  if (identificado) return identificado;
  return linha.match(/\b20\d{2}[./-]\d{2,6}\b/)?.[0];
}
...
function analisarFicheiro(texto: string, materiasPrimas: Array<Record<string, unknown>>): ManualResultado[] {
  return extrairLinhasFicheiro(texto).map((linha, index) => {
    const normalizada = linha.toLocaleLowerCase("pt-PT");
    ...
    return { id: String(index), linha, relevancia, correspondencias, url: extrairUrlOrigem(linha), prioridade: extrairPrioridade(linha), dataAlerta: extrairDataAlertaRasff(linha), numeroAlerta: extrairNumeroAlerta(linha) };
  });
}
...
function downloadExcel(results: ManualResultado[], fileName: string) {
  const rows = results.map((item, index) => ({
    ...
    "Nº alerta RASFF": item.numeroAlerta ?? "Não identificado",
...
export default function VigilanciaRasff() {
  ...
  const sugerirPrioridadeMutation = trpc.rasff.sugerirPrioridade.useMutation();
  const guardarAnaliseMutation = trpc.rasff.guardarAnaliseManual.useMutation({
    onSuccess: () => { toast.success("Análise manual guardada no histórico RASFF."); void reportsQuery.refetch(); },
    onError: error => toast.error(error.message),
  });
  ...
  const [manualFile, setManualFile] = useState<{ name: string; results: ManualResultado[]; totalLinhas: number } | null>(null);
  const [manualBusy, setManualBusy] = useState(false);
  const [iaBusyKey, setIaBusyKey] = useState<string | null>(null);
  const [sugestoesIa, setSugestoesIa] = useState<Record<string, { prioridade: PrioridadeAlerta; fundamento: string; confianca: number }>>({});
  ...
  const [mpFilter, setMpFilter] = useState("Todas");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const mpOptions = useMemo(() => {
    if (!manualFile) return [];
    return Array.from(new Set(manualFile.results.flatMap(item => item.correspondencias))).sort((a, b) => a.localeCompare(b, "pt-PT"));
  }, [manualFile]);
  const resultadosComPrioridade = useMemo(() => manualFile?.results.map(item => ({ ...item, prioridade: regrasPrioridade[chaveAprendizagem(item)] ?? item.prioridade })) ?? [], [manualFile, regrasPrioridade]);
  const filteredResults = useMemo(() => {
    if (!manualFile) return [];
    const resultados = resultadosComPrioridade.filter(item => {
      const matchesPriority = priorityFilter === "Todas" || item.prioridade === priorityFilter;
      const matchesDate = dataEstaNoIntervalo(item.dataAlerta, dateFrom, dateTo);
      const query = alertNumberFilter.trim().toLocaleLowerCase("pt-PT");
      const matchesAlertNumber = !query || [item.numeroAlerta, item.linha].filter(Boolean).some(value => String(value).toLocaleLowerCase("pt-PT").includes(query));
      const freeQuery = freeSearch.trim().toLocaleLowerCase("pt-PT");
      const searchableText = [item.linha, item.numeroAlerta, item.dataAlerta, item.prioridade, item.relevancia, item.url, ...item.correspondencias].filter(Boolean).join(" ").toLocaleLowerCase("pt-PT");
      const matchesFreeSearch = !freeQuery || searchableText.includes(freeQuery);
      const matchesMp = mpFilter === "Todas" ? true : mpFilter === "Sem correspondência" ? item.correspondencias.length === 0 : item.correspondencias.includes(mpFilter);
      return matchesPriority && matchesDate && matchesAlertNumber && matchesFreeSearch && matchesMp;
    });
    return resultados.sort((a, b) => {
      const timestampA = a.dataAlerta ? Date.parse(a.dataAlerta) : Number.NEGATIVE_INFINITY;
      const timestampB = b.dataAlerta ? Date.parse(b.dataAlerta) : Number.NEGATIVE_INFINITY;
      return timestampB - timestampA;
    });
  }, [manualFile, resultadosComPrioridade, priorityFilter, alertNumberFilter, freeSearch, mpFilter, dateFrom, dateTo]);
  ...
  const editarPrioridade = (item: ManualResultado, prioridade: PrioridadeAlerta) => {
    ...
    toast.success(`Prioridade ${prioridade} guardada e usada para aprendizagem assistida.`);
  };

  const chaveResultado = (item: ManualResultado) => `${item.numeroAlerta ?? "sem-numero"}:${item.linha}`;
  const pedirSugestaoIa = async (item: ManualResultado) => {
    if (!isQualidade) { toast.error("As sugestões de prioridade estão reservadas à Qualidade."); return; }
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível gerar a sugestão IA."); }
    finally { setIaBusyKey(null); }
  };

  const guardarAnaliseManual = () => {
    if (!manualFile || !isQualidade) return;
    const datas = manualFile.results.map(item => item.dataAlerta).filter((value): value is string => Boolean(value)).sort();
    const inicio = datas[0] ? `${datas[0]}T00:00:00.000Z` : undefined;
    const fim = datas[datas.length - 1] ? `${datas[datas.length - 1]}T23:59:59.999Z` : undefined;
    const relevantes = resultadosComPrioridade.filter(item => relevanciaOperacional(item) !== "Informativa");
    const resumo = `Análise manual do ficheiro ${manualFile.name}: ${manualFile.results.length} linhas/alertas lidos, ${filteredResults.length} apresentados pelos filtros no momento do arquivo, ${relevantes.length} relevantes. Período identificado: ${inicio ? inicio.slice(0, 10) : "não identificado"} a ${fim ? fim.slice(0, 10) : "não identificado"}.`;
    guardarAnaliseMutation.mutate({ ficheiro: manualFile.name, periodoInicio: inicio, periodoFim: fim, totalAvaliados: manualFile.results.length, totalRelevantes: relevantes.length, resumo, ocorrencias: resultadosComPrioridade, fontes: Array.from(new Set(resultadosComPrioridade.map(item => item.url).filter((url): url is string => Boolean(url))) ) });
  };
  ...
  const handleManualFile = async (file: File | undefined) => {
    ...
    try {
      ...
      const totalLinhas = texto.split(/\r?\n/).map(linha => linha.trim()).filter(linha => linha.length > 2).length;
      ...
      setAlertNumberFilter("");
      setMpFilter("Todas");
      setDateFrom("");
      setDateTo("");
      setManualFile({ name: file.name, results, totalLinhas });
  ...
  return (
    <SigaLayout title="Vigilância RASFF" subtitle="Monitorização de alertas relevantes para MP, fornecedores e origens">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" /> Analisar ficheiro RASFF</CardTitle><p className="mt-1 text-sm text-muted-foreground">Carregue um CSV, JSON, TXT, HTML ou Excel exportado para obter um resumo local em Excel, cruzar o conteúdo com as MP, fornecedores e origens do SIGA e abrir a fonte original de cada alerta quando existir uma URL.</p></div><label className="inline-flex cursor-pointer items-center justify-center rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground shadow-sm hover:opacity-90"><Upload className="mr-2 h-4 w-4" /> {manualBusy ? "A analisar…" : "Carregar ficheiro"}<input className="sr-only" type="file" accept=".csv,.json,.txt,.html,.htm,.xlsx,.xls,.xlsm,.ods,text/csv,application/json,text/plain,text/html,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" disabled={manualBusy} onChange={event => { handleManualFile(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label></CardHeader>
          <CardContent>{manualFile ? <div className="space-y-4"><div className="flex flex-col gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-emerald-900">{manualFile.name}</p><p className="mt-1 text-xs text-emerald-800">{manualFile.totalLinhas} linhas lidas · {manualFile.results.length} alertas reconhecidos · {filteredResults.length} apresentados · {manualFile.results.filter(item => !item.dataAlerta).length} sem data reconhecida · {filteredResults.filter(item => relevanciaOperacional(item) === "Direta").length} diretos · {filteredResults.filter(item => relevanciaOperacional(item) === "Indireta").length} indiretos</p></div><div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => downloadExcel(filteredResults, `resumo-rasff-${manualFile.name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.xlsx`)}><Download className="mr-2 h-4 w-4" /> Excel</Button>{isQualidade && <Button size="sm" variant="outline" onClick={guardarAnaliseManual} disabled={guardarAnaliseMutation.isPending}><CheckCircle2 className="mr-2 h-4 w-4" /> {guardarAnaliseMutation.isPending ? "A guardar…" : "Guardar no histórico"}</Button>}</div></div><div className="grid gap-3 rounded-xl border bg-slate-50/70 p-4 sm:grid-cols-4"><label className="text-xs font-medium text-slate-700">Prioridade<select value={priorityFilter} onChange={event => setPriorityFilter(event.target.value as "Todas" | PrioridadeAlerta)} className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm"><option value="Todas">Todas</option><option value="Alta">Alta</option><option value="Média">Média</option><option value="Baixa">Baixa</option><option value="Informativa">Informativa</option></select></label><label className="text-xs font-medium text-slate-700">MP afetada<select value={mpFilter} onChange={event => setMpFilter(event.target.value)} className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm"><option value="Todas">Todas</option>{mpOptions.map(mp => <option key={mp} value={mp}>{mp}</option>)}<option value="Sem correspondência">Sem correspondência</option></select></label><label className="text-xs font-medium text-slate-700">Pesquisa livre<input type="search" value={freeSearch} onChange={event => setFreeSearch(event.target.value)} placeholder="Pesquisar conteúdo, perigo, origem…" className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm" /></label><label className="text-xs font-medium text-slate-700">Nº alerta RASFF<input type="search" value={alertNumberFilter} onChange={event => setAlertNumberFilter(event.target.value)} placeholder="Ex.: 2026.1234" className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm" /></label><label className="text-xs font-medium text-slate-700">Data inicial<input type="date" value={dateFrom} onChange={event => setDateFrom(event.target.value)} className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm" /></label><label className="text-xs font-medium text-slate-700">Data final<input type="date" value={dateTo} onChange={event => setDateTo(event.target.value)} className="mt-1 block w-full rounded-md border bg-white px-3 py-2 text-sm" /></label></div><div className="max-h-80 space-y-2 overflow-y-auto">{filteredResults.length === 0 && <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-semibold">Nenhum alerta corresponde aos filtros atuais.</p><p className="mt-1 text-xs leading-5">O ficheiro contém {manualFile.totalLinhas} linhas lidas e {manualFile.results.length} alertas reconhecidos. Os restantes podem ser cabeçalhos, linhas vazias ou conteúdo sem estrutura de alerta; linhas sem data ficam identificadas no contador acima.</p><Button variant="outline" size="sm" className="mt-3" onClick={() => { setPriorityFilter("Todas"); setAlertNumberFilter(""); setFreeSearch(""); setMpFilter("Todas"); setDateFrom(""); setDateTo(""); }}>Limpar filtros</Button></div>}{filteredResults.map((item, index) => <div key={`${index}-${item.linha}`} className="rounded-lg border p-3"><div className="flex flex-wrap items-center gap-2">{item.numeroAlerta && <Badge variant="secondary">RASFF: {item.numeroAlerta}</Badge>}<Badge variant="outline" className={relevanciaOperacional(item) === "Direta" ? "border-red-200 bg-red-50 text-red-700" : relevanciaOperacional(item) === "Indireta" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-slate-200 bg-slate-50 text-slate-600"}>{relevanciaOperacional(item) === "Informativa" ? "Sem ação direta" : relevanciaOperacional(item)}</Badge>{item.correspondencias.map(match => <Badge key={match} variant="secondary">MP: {match}</Badge>)}<label className="inline-flex items-center gap-1 text-xs text-slate-600">Prioridade<select aria-label={`Editar prioridade do alerta ${item.numeroAlerta ?? index + 1}`} value={item.prioridade} onChange={event => editarPrioridade(item, event.target.value as PrioridadeAlerta)} disabled={!isQualidade} title={isQualidade ? "Editar e ensinar esta prioridade ao assistente" : "Apenas a Qualidade pode editar prioridades"} className="rounded-md border bg-white px-2 py-1 text-xs font-medium text-slate-800 disabled:cursor-not-allowed disabled:opacity-60"><option value="Alta">Alta</option><option value="Média">Média</option><option value="Baixa">Baixa</option><option value="Informativa">Informativa</option></select></label>{sugestaoAprendida(item, regrasPrioridade) && <Badge className="bg-violet-100 text-violet-700 hover:bg-violet-100">Aprendida</Badge>}<Button variant="outline" size="sm" disabled={!isQualidade || iaBusyKey === chaveResultado(item)} onClick={() => { void pedirSugestaoIa(item); }}>{iaBusyKey === chaveResultado(item) ? "A sugerir…" : "Sugestão IA"}</Button>{sugestoesIa[chaveResultado(item)] && <span className="inline-flex items-center gap-1 text-xs text-violet-700" title={sugestoesIa[chaveResultado(item)].fundamento}>Sugere {sugestoesIa[chaveResultado(item)].prioridade} · confiança {sugestoesIa[chaveResultado(item)].confianca}%<span className="h-1.5 w-12 overflow-hidden rounded-full bg-violet-100" aria-label={`Confiança ${sugestoesIa[chaveResultado(item)].confianca}%`}><span className={`block h-full rounded-full ${sugestoesIa[chaveResultado(item)].confianca >= 80 ? "bg-emerald-500" : sugestoesIa[chaveResultado(item)].confianca >= 60 ? "bg-amber-500" : "bg-red-400"}`} style={{ width: `${sugestoesIa[chaveResultado(item)].confianca}%` }} /></span><Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-violet-700" title={sugestoesIa[chaveResultado(item)].fundamento} onClick={() => editarPrioridade(item, sugestoesIa[chaveResultado(item)].prioridade)}>Aplicar</Button></span>}{item.url && <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md border border-primary/30 px-2 py-1 text-xs font-medium text-primary hover:bg-primary/5" aria-label={`Abrir fonte original do alerta ${index + 1}`}><ExternalLink className="h-3.5 w-3.5" /> Fonte original</a>}</div><p className="mt-2 break-words text-xs leading-5 text-slate-600">{item.linha}</p>{!item.url && <p className="mt-2 text-[11px] text-muted-foreground">Fonte original não identificada nesta linha.</p>}</div>)}</div><p className="text-xs leading-5 text-muted-foreground">Esta análise manual é uma triagem local e não cria uma decisão de conformidade. Confirme os resultados pela Qualidade e conserve o ficheiro original segundo o procedimento documental aplicável.</p></div> : <div className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">Ainda não foi carregado um ficheiro. O processamento ocorre no navegador, aceita CSV, JSON, TXT, HTML e folhas Excel, e limita-se a 5 MB.</div>}</CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>{reports.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">Ainda não existe um relatório. O primeiro será guardado após a execução semanal do agente.</div> : <div className="space-y-3">{reports.map(report => <div key={report.id} className="flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold">{report.origem === "manual" ? `Análise manual · ${report.ficheiroOrigem ?? report.nomeFicheiro}` : (report.codigoSemana ?? `Semana ${report.numeroSemana ?? "—"}`)}</span><Badge variant="outline" className={statusClass[report.estado] ?? ""}>{report.estado === "sucesso" ? <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> : null}{statusLabel[report.estado] ?? report.estado}</Badge></div><p className="mt-2 text-sm text-muted-foreground">{report.resumo}</p><p className="mt-2 text-xs text-muted-foreground">Gerado em {new Date(report.geradoEm).toLocaleString("pt-PT")} · Período {new Date(report.periodoInicio).toLocaleDateString("pt-PT")}–{new Date(report.periodoFim).toLocaleDateString("pt-PT")} · {report.totalAvaliados} avaliados · {report.totalRelevantes} relevantes</p>{report.origem === "manual" && Array.isArray(report.ocorrencias) && <Button size="sm" variant="outline" className="mt-3" onClick={() => downloadExcel(report.ocorrencias as ManualResultado[], `analise-rasff-${report.id}.xlsx`)}><Download className="mr-2 h-4 w-4" /> Descarregar Excel</Button>}</div></div>)}</div>}</CardContent>
        </Card>
      </div>
    </SigaLayout>
  );
