...
export default function VigilanciaRasff() {
  ...
  const manualSummary = useMemo(() => {
    ...
    return `# Resumo de análise RASFF — ${manualFile.name}\n\n- Alertas/linhas apresentados após filtros: ${filteredResults.length}\n- Filtro por número RASFF: ${alertNumberFilter || "todos"}\n- Filtro por MP afetada: ${mpFilter}\n- Correspondências diretas com MP do SIGA: ${diretas}\n- Correspondências indiretas/setoriais: ${indiretas}\n- Período de análise: ficheiro fornecido pela Qualidade\n\n## Resultados\n\n${filteredResults.map((item, index) => `${index + 1}. **${item.numeroAlerta ? `RASFF ${item.numeroAlerta}` : "Nº RASFF não identificado"}** · ${item.relevancia} · ${item.prioridade} · ${item.dataAlerta ?? "data não identificada"} — ${item.linha}${item.correspondencias.length ? `\\n   - MP correspondentes: ${item.correspondencias.join(", ")}` : ""}${item.url ? `\\n   - [Abrir fonte original](${item.url})` : ""}`).join("\\n")}`;
  }, [manualFile, filteredResults, alertNumberFilter, mpFilter]);
