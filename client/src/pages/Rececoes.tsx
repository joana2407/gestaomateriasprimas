...
import { normalizarTratamentoNaoConformidade, temTratamentoNaoConformidade, tratamentoNaoConformidadeVazio, type TratamentoNaoConformidade } from "../../../shared/rececao-nao-conformidade";
...
type RececaoForm = {
  ...
  tratamentoNaoConformidade: TratamentoNaoConformidade;
};
...
function emptyForm(responsavel = "", fabricaId = 0): RececaoForm {
  return {
    ...
    tratamentoNaoConformidade: tratamentoNaoConformidadeVazio(),
  };
}
...
export default function Rececoes() {
  ...
  const [rececaoParaValidar, setRececaoParaValidar] = useState<any | null>(null);
  const [decisaoValidacao, setDecisaoValidacao] = useState<"validada" | "recusada">("validada");
  const [justificacaoValidacao, setJustificacaoValidacao] = useState("");
  ...
  const upsert = trpc.rececoes.upsert.useMutation({
    onSuccess: data => {
      toast.success(data.estadoValidacao === "pendente" ? "Receção registada condicionalmente" : data.conformidade === "nao_conforme" ? "Receção registada como não conforme" : "Receção registada com sucesso");
      ...
      if (data.estadoValidacao === "pendente") toast.warning("A receção aguarda validação ou recusa justificada pela Qualidade.");
      if (data.notificacaoQualidadeEnviada) toast.info("A Qualidade foi notificada.");
      setDialogOpen(false);
      refetch();
    },
  ...
  const validarCondicional = trpc.rececoes.validarCondicional.useMutation({
    onSuccess: data => {
      toast.success(data.estadoValidacao === "validada" ? "Receção validada e disponibilizada para operação." : "Receção recusada com justificação registada.");
      setRececaoParaValidar(null);
      setJustificacaoValidacao("");
      refetch();
    },
    onError: error => toast.error(error.message),
  });
  ...
  function editarRececao(rececao: any) {
    setForm({
      ...
      tratamentoNaoConformidade: normalizarTratamentoNaoConformidade(rececao.tratamentoNaoConformidade as Partial<TratamentoNaoConformidade> | null),
    });
    setDialogOpen(true);
  }
  ...
  function setTratamentoNaoConformidade<K extends keyof TratamentoNaoConformidade>(key: K, value: TratamentoNaoConformidade[K]) {
    setForm(current => ({
      ...current,
      tratamentoNaoConformidade: { ...current.tratamentoNaoConformidade, [key]: value },
    }));
  }
  ...
  function guardar() {
    ...
    upsert.mutate({
      ...
      tratamentoNaoConformidade: temTratamentoNaoConformidade(form.tratamentoNaoConformidade) ? form.tratamentoNaoConformidade : null,
    });
  }
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        {user?.role === "qualidade" && rececoesCondicionais.length > 0 && <section className="overflow-hidden rounded-xl border border-amber-300 bg-amber-50/60"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-amber-200 px-4 py-3"><div className="flex items-center gap-2 text-amber-950"><AlertTriangle className="h-4 w-4" /><div><p className="text-sm font-semibold">Receções a validar</p><p className="text-[11px] text-amber-800">Estas receções permanecem inacessíveis operacionalmente até decisão justificada.</p></div></div><Badge className="bg-amber-600 text-white hover:bg-amber-600">{rececoesCondicionais.length} pendente(s)</Badge></div><div className="divide-y divide-amber-200">{rececoesCondicionais.map(rececao => <div key={rececao.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-foreground">{rececao.materiaPrimaNome} {rececao.lote ? `· Lote ${rececao.lote}` : ""}</p><p className="mt-0.5 text-xs text-amber-900">{rececao.fornecedorNome} · {rececao.motivoNaoConformidade ? "Não conformidade registada" : "Validade abaixo do mínimo"}</p></div><Button size="sm" className="gap-1.5 self-start sm:self-auto" onClick={() => { setRececaoParaValidar(rececao); setDecisaoValidacao("validada"); setJustificacaoValidacao(""); }}><ClipboardCheck className="h-3.5 w-3.5" />Validar ou recusar</Button></div>)}</div></section>}
        ...
        <div className="space-y-2">
          {filteredRececoes.length === 0 ? <div className="card-elegant p-12 text-center"><ClipboardCheck className="w-10 h-10 mx-auto mb-3 text-muted-foreground/30" /><p className="text-sm font-medium">Ainda não existem receções neste filtro</p><p className="text-xs text-muted-foreground mt-1">Registe a primeira receção para iniciar o controlo de conformidade.</p></div> : filteredRececoes.map(rececao => {
            ...
            return <div key={rececao.id} className="card-elegant p-4 flex flex-col lg:flex-row lg:items-center gap-4">
              ...
              <div className="flex items-center gap-2 justify-between lg:justify-end flex-wrap"><span className={cn("inline-flex items-center gap-1.5 border rounded-full px-2.5 py-1 text-xs font-medium", config.className)}><Icon className="w-3.5 h-3.5" />{config.label}</span>{rececao.estadoValidacao === "pendente" && <Badge className="bg-amber-600 text-white hover:bg-amber-600">Aguarda validação</Badge>}{rececao.estadoValidacao === "validada" && <Badge className="bg-emerald-600 text-white hover:bg-emerald-600">Validada por {rececao.validadoPorNome ?? "Qualidade"}</Badge>}{rececao.estadoValidacao === "recusada" && <Badge variant="destructive">Recusada por {rececao.validadoPorNome ?? "Qualidade"}</Badge>}<Button variant="outline" size="sm" className="gap-1.5" onClick={() => setRececaoDetalheId(rececao.id)}><Eye className="w-3.5 h-3.5" />Detalhe do lote</Button>{podeTransferirStock && rececao.estadoValidacao !== "pendente" && rececao.estadoValidacao !== "recusada" && rececao.lote && (quantidadeTransferidaPorRececao.get(rececao.id) ?? 0) < rececao.quantidade && <Button size="icon" title="Transferir lote" aria-label="Transferir lote" onClick={() => abrirTransferenciaStock(rececao)}><ArrowRightLeft className="w-4 h-4" /></Button>}{podeEditarRececaoPorUtilizador({ role: user?.role, userId: user?.id, registadoPor: rececao.registadoPor }) && <button type="button" onClick={() => editarRececao(rececao)} className="p-2 rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground" title="Editar receção"><Edit2 className="w-4 h-4" /></button>}{podeEliminar && <button type="button" onClick={() => setRececaoParaEliminar(rececao)} className="p-2 rounded-lg text-red-600 hover:bg-red-50" title="Eliminar receção"><Trash2 className="w-4 h-4" /></button>}</div>
            </div>;
          })}
        </div>
      ...
      <Dialog open={Boolean(rececaoParaValidar)} onOpenChange={open => { if (!open) { setRececaoParaValidar(null); setJustificacaoValidacao(""); } }}>
        <DialogContent className="w-[calc(100vw-1rem)] max-w-lg">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><ClipboardCheck className="h-5 w-5 text-primary" />Decidir receção condicional</DialogTitle><DialogDescription>Registe a validação ou a recusa e justifique a decisão. Esta ação fica gravada no histórico da receção.</DialogDescription></DialogHeader>
          {rececaoParaValidar && <div className="space-y-4 pt-2"><div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm"><p className="font-semibold text-amber-950">{rececaoParaValidar.materiaPrimaNome} {rececaoParaValidar.lote ? `· Lote ${rececaoParaValidar.lote}` : ""}</p><p className="mt-1 text-xs text-amber-900">{rececaoParaValidar.fornecedorNome} · Conformidade: {rececaoParaValidar.conformidade === "nao_conforme" ? "Não conforme" : "Validade abaixo do mínimo"}</p></div><div className="grid grid-cols-2 gap-2"><Button type="button" variant={decisaoValidacao === "validada" ? "default" : "outline"} onClick={() => setDecisaoValidacao("validada")}>Validar receção</Button><Button type="button" variant={decisaoValidacao === "recusada" ? "destructive" : "outline"} onClick={() => setDecisaoValidacao("recusada")}>Recusar receção</Button></div><Field label={`Fundamento da ${decisaoValidacao === "validada" ? "aceitação" : "recusa"} *`}><Textarea value={justificacaoValidacao} onChange={event => setJustificacaoValidacao(event.target.value)} placeholder={decisaoValidacao === "validada" ? "Indique a informação que fundamenta a aceitação..." : "Indique a informação que fundamenta a recusa e a ação a tomar..."} /><p className="mt-1 text-[11px] text-muted-foreground">Indique pelo menos {MINIMO_CARACTERES_FUNDAMENTO_DECISAO} caracteres.</p></Field><div className="flex flex-col-reverse gap-2 border-t border-border/60 pt-3 sm:flex-row sm:justify-end"><Button variant="outline" onClick={() => { setRececaoParaValidar(null); setJustificacaoValidacao(""); }}>Cancelar</Button><Button variant={decisaoValidacao === "recusada" ? "destructive" : "default"} disabled={validarCondicional.isPending || justificacaoValidacao.trim().length < MINIMO_CARACTERES_FUNDAMENTO_DECISAO} onClick={() => validarCondicional.mutate({ id: rececaoParaValidar.id, decisao: decisaoValidacao, justificacao: justificacaoValidacao.trim() })}>{validarCondicional.isPending ? "A guardar..." : decisaoValidacao === "validada" ? "Confirmar validação" : "Confirmar recusa"}</Button></div></div>}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="w-[calc(100vw-1rem)] sm:w-[calc(100vw-2rem)] max-w-6xl max-h-[92vh] overflow-y-auto p-4 sm:p-6">
          ...
          <div className="space-y-6 pt-2">
            ...
            {conformidadeCalculada === "nao_conforme" && <div className="space-y-4 rounded-xl border border-red-200 bg-red-50 p-4"><div className="flex items-center gap-2 text-red-800"><AlertTriangle className="w-4 h-4" /><p className="text-sm font-semibold">Tratamento de não conformidade obrigatório</p></div><Field label="Não conformidade identificada *"><Textarea value={form.motivoNaoConformidade} onChange={event => setForm(current => ({ ...current, motivoNaoConformidade: event.target.value }))} placeholder="Descreva a não conformidade identificada..." className="bg-background" /></Field>{user?.role === "qualidade" && <div className="space-y-4 border-t border-red-200 pt-4"><div><p className="text-sm font-semibold text-red-950">Tratamento pela Qualidade</p><p className="mt-0.5 text-xs text-red-800">Registe a correção e a ação corretiva, incluindo responsáveis, prazos e evidências.</p></div><div className="grid gap-4 lg:grid-cols-2"><section className="space-y-3 rounded-lg border border-red-200 bg-background/70 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-red-800">Correção imediata</p><Field label="Correção aplicada"><Textarea value={form.tratamentoNaoConformidade.correcaoImediata} onChange={event => setTratamentoNaoConformidade("correcaoImediata", event.target.value)} placeholder="Medida tomada para controlar a situação..." /></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Responsável"><Input value={form.tratamentoNaoConformidade.responsavelCorrecao} onChange={event => setTratamentoNaoConformidade("responsavelCorrecao", event.target.value)} placeholder="Nome" /></Field><Field label="Data de implementação"><Input type="date" value={form.tratamentoNaoConformidade.dataImplementacaoCorrecao} onChange={event => setTratamentoNaoConformidade("dataImplementacaoCorrecao", event.target.value)} /></Field></div><Field label="Evidência da correção"><Textarea value={form.tratamentoNaoConformidade.evidenciaCorrecao} onChange={event => setTratamentoNaoConformidade("evidenciaCorrecao", event.target.value)} placeholder="Referência, fotografia, documento ou outra evidência..." /></Field></section><section className="space-y-3 rounded-lg border border-red-200 bg-background/70 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-red-800">Ação corretiva</p><Field label="Ação corretiva"><Textarea value={form.tratamentoNaoConformidade.acaoCorretiva} onChange={event => setTratamentoNaoConformidade("acaoCorretiva", event.target.value)} placeholder="Ação para evitar a repetição da não conformidade..." /></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Responsável"><Input value={form.tratamentoNaoConformidade.responsavelAcaoCorretiva} onChange={event => setTratamentoNaoConformidade("responsavelAcaoCorretiva", event.target.value)} placeholder="Nome" /></Field><Field label="Prazo de implementação"><Input type="date" value={form.tratamentoNaoConformidade.prazoAcaoCorretiva} onChange={event => setTratamentoNaoConformidade("prazoAcaoCorretiva", event.target.value)} /></Field><Field label="Data de fecho"><Input type="date" value={form.tratamentoNaoConformidade.dataFechoAcaoCorretiva} onChange={event => setTratamentoNaoConformidade("dataFechoAcaoCorretiva", event.target.value)} /></Field></div><Field label="Evidência da ação corretiva"><Textarea value={form.tratamentoNaoConformidade.evidenciaAcaoCorretiva} onChange={event => setTratamentoNaoConformidade("evidenciaAcaoCorretiva", event.target.value)} placeholder="Referência, fotografia, documento ou outra evidência..." /></Field></section></div></div>}</div>}
      ...
      <Dialog open={Boolean(rececaoDetalhe)} onOpenChange={open => !open && setRececaoDetalheId(null)}>
        <DialogContent className="w-[calc(100vw-1rem)] max-w-3xl max-h-[92vh] overflow-y-auto p-4 sm:p-6">
          ...
          {rececaoDetalhe && <div className="space-y-5 pt-3"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-xl border border-border/70 bg-muted/20 p-4 text-sm"><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Matéria-prima</p><p className="mt-1 font-semibold">{rececaoDetalhe.materiaPrimaNome}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Lote</p><p className="mt-1 font-semibold">{rececaoDetalhe.lote || "Não indicado"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Recebido</p><p className="mt-1">{rececaoDetalhe.quantidade} {formatarUnidadeRececao(rececaoDetalhe.unidade as UnidadeRececao)} · {new Date(rececaoDetalhe.dataRececao).toLocaleDateString("pt-PT")}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Validade do produto</p><p className="mt-1">{formatarValidadeRececao(rececaoDetalhe.validade)}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Responsável pela receção</p><p className="mt-1">{rececaoDetalhe.responsavel}</p></div></div>{(rececaoDetalhe.estadoValidacao === "validada" || rececaoDetalhe.estadoValidacao === "recusada") && <section className={cn("overflow-hidden rounded-xl border", rececaoDetalhe.estadoValidacao === "validada" ? "border-emerald-200" : "border-red-200")}><div className={cn("border-b px-4 py-3", rececaoDetalhe.estadoValidacao === "validada" ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50")}><p className={cn("text-sm font-semibold", rececaoDetalhe.estadoValidacao === "validada" ? "text-emerald-900" : "text-red-900")}>{rececaoDetalhe.estadoValidacao === "validada" ? "Receção validada pela Qualidade" : "Receção recusada pela Qualidade"}</p></div><div className="grid gap-3 p-4 text-sm sm:grid-cols-2"><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Decidido por</p><p className="mt-1 font-medium">{rececaoDetalhe.validadoPorNome ?? "Equipa de Qualidade"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Data da decisão</p><p className="mt-1">{rececaoDetalhe.validadoEm ? new Date(rececaoDetalhe.validadoEm).toLocaleString("pt-PT") : "Não indicada"}</p></div><div className="sm:col-span-2"><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Fundamento da {rececaoDetalhe.estadoValidacao === "validada" ? "aceitação" : "recusa"}</p><p className="mt-1 whitespace-pre-wrap">{rececaoDetalhe.motivoValidacaoCondicional || "Não indicado"}</p></div></div></section>}{rececaoDetalhe.motivoNaoConformidade && <section className="overflow-hidden rounded-xl border border-red-200"><div className="border-b border-red-200 bg-red-50 px-4 py-3"><p className="text-sm font-semibold text-red-900">Tratamento da não conformidade</p></div><div className="space-y-4 p-4 text-sm"><div><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Não conformidade identificada</p><p className="mt-1 whitespace-pre-wrap">{rececaoDetalhe.motivoNaoConformidade}</p></div>{temTratamentoNaoConformidade(rececaoDetalhe.tratamentoNaoConformidade as Partial<TratamentoNaoConformidade> | null) ? <div className="grid gap-4 lg:grid-cols-2"><div className="space-y-2 rounded-lg bg-muted/30 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Correção imediata</p><p className="whitespace-pre-wrap">{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).correcaoImediata || "Não indicada"}</p><p className="text-xs text-muted-foreground">Responsável: {(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).responsavelCorrecao || "Não indicado"}{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).dataImplementacaoCorrecao ? ` · ${formatarValidadeRececao((rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).dataImplementacaoCorrecao)}` : ""}</p>{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).evidenciaCorrecao && <p className="text-xs text-muted-foreground">Evidência: {(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).evidenciaCorrecao}</p>}</div><div className="space-y-2 rounded-lg bg-muted/30 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Ação corretiva</p><p className="whitespace-pre-wrap">{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).acaoCorretiva || "Não indicada"}</p><p className="text-xs text-muted-foreground">Responsável: {(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).responsavelAcaoCorretiva || "Não indicado"}{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).prazoAcaoCorretiva ? ` · Prazo ${formatarValidadeRececao((rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).prazoAcaoCorretiva)}` : ""}</p>{(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).evidenciaAcaoCorretiva && <p className="text-xs text-muted-foreground">Evidência: {(rececaoDetalhe.tratamentoNaoConformidade as TratamentoNaoConformidade).evidenciaAcaoCorretiva}</p>}</div></div> : <p className="rounded-lg bg-muted/30 p-3 text-xs text-muted-foreground">A aguardar registo do tratamento pela equipa de Qualidade.</p>}</div></section>}<DetalheValidacoesRececao validacoes={validacoesDoLote} /><section className="rounded-xl border border-border/70 overflow-hidden"><div className="flex items-center gap-2 border-b border-border/60 px-4 py-3"><ArrowRightLeft className="w-4 h-4 text-primary" /><div><p className="text-sm font-semibold">Histórico completo de transferências</p><p className="text-[11px] text-muted-foreground">{transferenciasDoLote.length} movimento(s) associado(s) a este lote.</p></div></div><div className="divide-y divide-border/60">{transferenciasDoLote.length ? transferenciasDoLote.map(transferencia => { const destino = fabricas?.find(fabrica => fabrica.id === transferencia.fabricaDestinoId); return <div key={transferencia.id} className="p-4 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-medium">{transferencia.quantidade} {formatarUnidadeRececao(transferencia.unidade as UnidadeRececao)} para {destino?.nome ?? "Fábrica de destino"}</p><span className="text-xs text-muted-foreground">{new Date(transferencia.dataTransferencia).toLocaleDateString("pt-PT")}</span></div><p className="mt-1 text-xs text-muted-foreground">Responsável: {transferencia.responsavel} · Motivo: {transferencia.motivo}</p>{transferencia.observacoes && <p className="mt-1 text-xs text-muted-foreground">Observações: {transferencia.observacoes}</p>}</div>; }) : <p className="p-5 text-sm text-muted-foreground">Ainda não existem transferências associadas a este lote.</p>}</div></section>{podeTransferirStock && rececaoDetalhe.estadoValidacao !== "pendente" && rececaoDetalhe.estadoValidacao !== "recusada" && rececaoDetalhe.lote && (quantidadeTransferidaPorRececao.get(rececaoDetalhe.id) ?? 0) < rececaoDetalhe.quantidade && <div className="flex justify-end"><Button size="icon" title="Transferir deste lote" aria-label="Transferir deste lote" onClick={() => { setRececaoDetalheId(null); abrirTransferenciaStock(rececaoDetalhe); }}><ArrowRightLeft className="w-4 h-4" /></Button></div>}</div>}
        </DialogContent>
      </Dialog>
