      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="w-[calc(100vw-1rem)] sm:w-[calc(100vw-2rem)] max-w-6xl max-h-[92vh] overflow-y-auto p-4 sm:p-6">
          ...
          <div className="space-y-6 pt-2">
            <section className="rounded-xl border border-border/70 overflow-hidden"><div className="px-4 py-3 bg-muted/40 flex items-center gap-2"><Warehouse className="w-4 h-4 text-primary" /><p className="text-sm font-semibold">Destino e identificação</p></div><div className="p-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <section className="rounded-xl border border-border/70 overflow-hidden"><div className="px-4 py-3 bg-muted/40 flex items-center gap-2"><PackageCheck className="w-4 h-4 text-primary" /><p className="text-sm font-semibold">Matéria-prima e lote</p></div><div className="p-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="space-y-1.5 min-w-0"><label className="block text-xs font-medium text-muted-foreground break-words">{label}</label>{children}</div>;
}
